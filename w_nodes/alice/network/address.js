...
class ADRESS {
  ...
  getPubKey (){
    let key = fs.readFileSync("./.key/publicKey.pem", "utf8")
    let pubKey = crypto.createPublicKey({key, type: "spki", format: "pem" });
  ...
  create(){
    let filepublicKey = this.getPubKey();
    let publicKey = filepublicKey.export({
  ...
  verify ({ publicKey }){
    let address = UTIL.hash(publicKey);
